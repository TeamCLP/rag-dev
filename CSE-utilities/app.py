...
load_dotenv()  # charge les variables du fichier .env s'il est présent
...
# Config
# ============================
EMBED_MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"  # léger & rapide
CHUNK_SIZE = 600
CHUNK_OVERLAP = 120
TOP_K = 5
...
# Index global + stockage des chunks
faiss_index = None
sk_index = None
emb_matrix = None
chunks: List[str] = []
...
def _read_path(path: str) -> str:
    """Lit un chemin de fichier (bytes) et retourne le texte extrait."""
...
# ============================
# Indexation (FAISS ou sklearn)
# ============================
def build_index(all_chunks: List[str]):
    """Construit l'index sémantique à partir des chunks."""
...
def search_similar(query: str, k: int = TOP_K) -> List[Tuple[int, float]]:
    """Retourne [(index_chunk, score_similarité), ...]"""
...
async def answer_query(query: str, backend: str, model_choice: str):
    ...
    if backend == "Ollama (local)":
        model = (model_choice or OLLAMA_MODEL_DEFAULT).strip()
        reply = await chat_ollama(model, RAG_SYSTEM, user_prompt)
    else:
        model = (model_choice or OPENAI_MODEL_DEFAULT).strip()
...
def handle_upload(file_paths: List[str]):
    if not file_paths:
        return "Aucun fichier reçu."
    texts = []
    for path in file_paths:  # liste de chemins (str)
        name = os.path.basename(path)
        try:
            txt = _read_path(path)
            texts.append(f"\n\n===== {name} =====\n\n" + txt)
        except Exception as e:
            texts.append(f"\n\n===== {name} =====\n\n[Erreur lecture {name}: {e}]")
    if not texts:
        return "Aucun texte exploitable. Formats acceptés: PDF, DOCX, TXT."
    merged = "\n\n".join(texts)
    return f"{len(merged)} caractères chargés. Cliquez sur 'Indexer'."

def build_chunks_and_index(file_paths: List[str]):
    global chunks
    if not file_paths:
...
with gr.Blocks(title="RAG Local — Débutant", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        files = gr.File(
            ...
            type="filepath",          # <<< chemins de fichiers (évite NamedString)
            height=120
        )
    ...
    with gr.Accordion("Paramètres IA", open=False):
        ...
        **OpenAI** : créez un fichier `.env` avec `OPENAI_API_KEY=sk-...`.
        """)
...
if __name__ == "__main__":
    # Si besoin, ajuste server_name/server_port ; mets share=True si localhost est bloqué chez toi
